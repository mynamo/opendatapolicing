						...
						putcopySiteStateResponse(siteRequest, c -> {
							if(c.succeeded()) {
								...
								workerExecutor.executeBlocking(
									blockingCodeHandler -> {
										try {
											aSearchSiteState(siteRequest, false, true, "/api/state/copy", "PUTCopy", d -> {
												if(d.succeeded()) {
			...
			if(jsonObject != null) {
				...
				for(Integer i = 0; i < entityVars.size(); i++) {
					...
					case "stateName":
						futures.add(Future.future(a -> {
							tx.preparedQuery(SiteContextEnUS.SQL_setD
									, Tuple.of(pk, "stateName", Optional.ofNullable(jsonObject.getValue(entityVar)).map(s -> s.toString()).orElse(null))
									, b
							-> {
								if(b.succeeded())
									a.handle(Future.succeededFuture());
					...
					case "stateAbbreviation":
						futures.add(Future.future(a -> {
							tx.preparedQuery(SiteContextEnUS.SQL_setD
									, Tuple.of(pk, "stateAbbreviation", Optional.ofNullable(jsonObject.getValue(entityVar)).map(s -> s.toString()).orElse(null))
									, b
							-> {
								if(b.succeeded())
									a.handle(Future.succeededFuture());
				...
				switch(methodName) {
					...
					case "setStateName":
						if(jsonObject.getString(methodName) == null) {
							...
							futures.add(Future.future(a -> {
								tx.preparedQuery(SiteContextEnUS.SQL_setD
										, Tuple.of(pk, "stateName", o2.jsonStateName())
										, b
								-> {
									if(b.succeeded())
										a.handle(Future.succeededFuture());
					...
					case "setStateAbbreviation":
						if(jsonObject.getString(methodName) == null) {
							...
							futures.add(Future.future(a -> {
								tx.preparedQuery(SiteContextEnUS.SQL_setD
										, Tuple.of(pk, "stateAbbreviation", o2.jsonStateAbbreviation())
										, b
								-> {
									if(b.succeeded())
										a.handle(Future.succeededFuture());
		...
		try {
			{
				userSiteState(siteRequest, b -> {
					if(b.succeeded()) {
						aSearchSiteState(siteRequest, false, true, "/api/state/{id}", "GET", c -> {
							if(c.succeeded()) {
		...
		try {
			{
				userSiteState(siteRequest, b -> {
					if(b.succeeded()) {
						aSearchSiteState(siteRequest, false, true, "/api/admin/state", "AdminSearch", c -> {
							if(c.succeeded()) {
								SearchList<SiteState> listSiteState = c.result();
		...
		try {
			{
				userSiteState(siteRequest, b -> {
					if(b.succeeded()) {
						aSearchSiteState(siteRequest, false, true, "/state", "SearchPage", c -> {
							if(c.succeeded()) {
								SearchList<SiteState> listSiteState = c.result();
