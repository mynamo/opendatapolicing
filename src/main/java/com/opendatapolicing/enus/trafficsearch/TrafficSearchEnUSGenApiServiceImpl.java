						...
						putcopyTrafficSearchResponse(siteRequest, c -> {
							if(c.succeeded()) {
								...
								workerExecutor.executeBlocking(
									blockingCodeHandler -> {
										try {
											aSearchTrafficSearch(siteRequest, false, true, "/api/traffic-search/copy", "PUTCopy", d -> {
												if(d.succeeded()) {
		...
		try {
			{
				userTrafficSearch(siteRequest, b -> {
					if(b.succeeded()) {
						aSearchTrafficSearch(siteRequest, false, true, "/api/traffic-search/{id}", "GET", c -> {
							if(c.succeeded()) {
		...
		try {
			{
				userTrafficSearch(siteRequest, b -> {
					if(b.succeeded()) {
						aSearchTrafficSearch(siteRequest, false, true, "/api/admin/traffic-search", "AdminSearch", c -> {
							if(c.succeeded()) {
								SearchList<TrafficSearch> listTrafficSearch = c.result();
		...
		try {
			{
				userTrafficSearch(siteRequest, b -> {
					if(b.succeeded()) {
						aSearchTrafficSearch(siteRequest, false, true, "/traffic-search", "SearchPage", c -> {
							if(c.succeeded()) {
								SearchList<TrafficSearch> listTrafficSearch = c.result();
